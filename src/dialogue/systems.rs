use super::intel::*;
use super::nodes::{Comparison, DialogueCondition, DialogueEffect, DialogueTree, EmotionType};
use super::patrons::{Anger, Happiness, Patron as PatronComponent, Sadness};
use bevy::prelude::*;
...
impl Plugin for DialogueInteractionPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<DialogueOptionSelected>().add_systems(
            Update,
            (handle_dialogue_option_selection, update_dialogue_ui),
        );
    }
}
        ...
        if let Ok((_entity, mut active_dialogue)) = dialogue_query.get_mut(event.dialogue_entity) {
            if let Some(dialogue_tree) = dialogue_assets.get(&active_dialogue.tree_handle) {
                if let Some(current_node) =
                    dialogue_tree.nodes.get(&active_dialogue.current_node_id)
                {
                        ...
                        for effect in &selected_option.effects {
                            match effect {
                                DialogueEffect::SetIntel(intel_id) => {
                                    ...
                                    intel_events.write(IntelDiscoveredEvent {
                                ...
                                DialogueEffect::ModifyEmotion(patron_name, emotion_type, value) => {
                                    ...
                                    for (patron, mut happiness, mut sadness, mut anger) in
                                        patrons_query.iter_mut()
                                    {
                                        if patron.name == *patron_name {
                                            // Apply the emotion change
                                            match emotion_type {
                                                EmotionType::Happiness => {
                                                    let new_value = (happiness.value as i16 + value)
                                                        .clamp(0, 100)
                                                        as u8;
                                                    happiness.value = new_value;
                                                    info!(
                                                        "Patron {} happiness changed to {}",
                                                        patron_name, new_value
                                                    );
                                                }
                                                EmotionType::Sadness => {
                                                    let new_value = (sadness.value as i16 + value)
                                                        .clamp(0, 100)
                                                        as u8;
                                                    sadness.value = new_value;
                                                    info!(
                                                        "Patron {} sadness changed to {}",
                                                        patron_name, new_value
                                                    );
                                                }
                                                EmotionType::Anger => {
                                                    let new_value = (anger.value as i16 + value)
                                                        .clamp(0, 100)
                                                        as u8;
                                                    anger.value = new_value;
                                                    info!(
                                                        "Patron {} anger changed to {}",
                                                        patron_name, new_value
                                                    );
                                                }
                                            }
                ...
                for (i, option) in current_node.options.iter().enumerate() {
                    let is_available = option.conditions.iter().all(|condition| {
                        match condition {
                            ...
                            DialogueCondition::EmotionCheck(
                                patron_name,
                                emotion_type,
                                threshold,
                                comparison,
