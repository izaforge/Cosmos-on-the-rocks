...
pub fn spawn_dialogue_runner(
    ...
    let calming_value = *patron_effects
        .effects
        .get(&PrimaryEffect::Calming)
        .unwrap_or(&0) as f32;
    let energizing_value = *patron_effects
        .effects
        .get(&PrimaryEffect::Energizing)
        .unwrap_or(&0) as f32;
    let mind_enhancing_value = *patron_effects
        .effects
        .get(&PrimaryEffect::MindEnhancing)
        .unwrap_or(&0) as f32;
    let courage_value = *patron_effects
        .effects
        .get(&PrimaryEffect::CourageBoosting)
        .unwrap_or(&0) as f32;
    let truth_value = *patron_effects
        .effects
        .get(&PrimaryEffect::TruthInducing)
        .unwrap_or(&0) as f32;
    let healing_value = *patron_effects
        .effects
        .get(&PrimaryEffect::Healing)
        .unwrap_or(&0) as f32;
    ...
    variables.insert(
        "$mind_enhancing_effect".to_string(),
        mind_enhancing_value.into(),
    );
    ...
    println!(
        "?? Set initial variables - energizing: {}, truth: {}, mind: {}",
    ...
    for (drink_entity, drink) in drinks.iter() {
        ...
        if drink.target_patron.is_none() {
            ...
            for (effect, value) in &drink.effects {
                ...
                info!(
                    "? Applied {:?} effect: {} -> {}",
                    effect, current_value, new_value
                );
            }
    ...
    for (drink_entity, drink) in drinks.iter() {
        ...
        for (ingredient_entity, amount) in &drink.ingredients {
            if let Ok(ingredient) = ingredients.get(*ingredient_entity) {
                ...
                info!(
                    "Ingredient {} contributes {:?} effect with strength {}",
...
fn update_dialogue_variables(
    ...
    for mut dialogue_runner in dialogue_runner_query.iter_mut() {
        ...
        debug!(
            "?? Updated dialogue variables - energizing: {}, truth: {}, mind: {}",
            energizing_value, truth_value, mind_enhancing_value
        );
    }
}
